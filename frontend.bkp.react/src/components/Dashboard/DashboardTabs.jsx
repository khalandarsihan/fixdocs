...
//     // Use below div for horizontal stack
...
//                       {alert.document_type || alert.documentType || alert.name || alert.id}
//                     </p>
//                     <p className="text-sm text-gray-500">
//                       {alert.document_id || alert.documentId} - 
//                       {alert.date_of_expiry || alert.dateOfExpiry ? 
//                         ` Expires: ${formatDate(alert.date_of_expiry || alert.dateOfExpiry)}` : 
//                         ''}
//                     </p>
//                   </div>
//                 </div>
          ...
          <div className="p-4">
            {isLoading ? (
              ...
              <div className="space-y-2 max-h-[500px] overflow-y-auto">
                ...
                <button className="w-full py-2 px-4 bg-gray-100 hover:bg-gray-200 rounded-md text-sm font-medium mt-2">
                  View all works
                </button>
              </div>
            ) : (
...
function AlertsTab({ filteredAlerts, isLoading }) {
  // Calculate the number of alerts for UI feedback
  const alertCount = filteredAlerts ? filteredAlerts.length : 0;
    ...
    <div className="bg-white rounded-lg border shadow-sm">
      <div className="p-4 border-b">
        ...
        <p className="text-sm text-gray-500 mt-1">
          {alertCount > 0 ? `Showing ${alertCount} alerts` : "No alerts to display"}
        </p>
      </div>
      <div className="p-4">
        {isLoading ? (
                ...
                {filteredAlerts.map((alert, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-sm">
                      {alert.alert_type || alert.alertType || ''}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm">
                      {(alert.document_type || alert.documentType || '')} {(alert.document_id || alert.documentId || '')}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm">
                      {alert.bill_to || alert.billTo || alert.entity || ''}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm">
                      {(alert.date_of_expiry || alert.dateOfExpiry) ? 
                        formatDate(alert.date_of_expiry || alert.dateOfExpiry) : 
                        'No date'}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm">
          ...
          <div className="text-center py-8 text-gray-500">
            <p>No alerts found</p>
            <p className="text-xs mt-2">Check the API response format or try refreshing the page</p>
          </div>
        )}
      </div>
    </div>
